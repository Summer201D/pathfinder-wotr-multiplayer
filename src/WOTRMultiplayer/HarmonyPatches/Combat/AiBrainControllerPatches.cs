    ...
    public class AiBrainControllerPatches
    {
        ...
        private static int RollAIActionCooldown(DiceFormula diceFormula, AiAction aiAction, UnitCommand unitCommand)
        {
            ...
            try
            {
                ...
                var identifier = $"{nameof(CombatAiData)}:{nameof(CombatAiData.UseAction)}:{nameof(RollAIActionCooldown)}:{sessionSeed}:{combatSeed}:{Game.Instance.Player.GameId}:{unitCommand.Executor.UniqueId}:{aiAction.Blueprint.AssetGuid}:{aiAction.Blueprint.name}:{unitCommand.TargetUnit?.UniqueId}";
