    ...
    public class RandomLootPatches
    {
        ...
        private static Func<int, int, int> GetBlueprintLootRandomizator(BlueprintLoot blueprintLoot)
        {
            ...
            try
            {
                ...
                var identifier = $"{nameof(BlueprintLoot)}:{nameof(GetBlueprintLootRandomizator)}:{Game.Instance.Player.GameId}:{blueprintLoot.Area?.name}:{blueprintLoot.Area?.AssetGuid.ToString()}:{blueprintLoot.Cost}:{sessionSeed.Value}";
        ...
        private static Func<int, int, int> GetContextActionLootRandomizator(ContextActionAddRandomTrashItem contextActionAddRandom)
        {
            ...
            {
                var sessionSeed = Main.Multiplayer.GetSessionSeed();
                var identifier = $"{nameof(ContextActionAddRandomTrashItem)}:{nameof(GetContextActionLootRandomizator)}:{Game.Instance.Player.GameId}:{contextActionAddRandom.m_LootType}:{contextActionAddRandom.m_MaxCost}:{contextActionAddRandom.m_Silent}:{sessionSeed.Value}";
                var randomizer = Main.Multiplayer.ValueGenerator.GetRandom(SeedLifetime.Area, identifier);
                Main.GetLogger<RandomLootPatches>().LogInformation("Randomizer for ContextActionAddRandomTrashItem has been initialized. Identifier={Identifier}", identifier);
                return randomizer.Next;
            }
            catch (Exception ex)
            {
        ...
        private static int GetRandomLootItemsPack(int minInclusive, int maxExclusive, LootItemsPackVariable lootRandomItem)
        {
            ...
            try
            {
                ...
                var identifier = $"{nameof(LootItemsPackVariable)}:{nameof(GetRandomLootItemsPack)}:{Game.Instance.Player.GameId}:{ownerBlueprintId}:{ownerBlueprintName}:{sessionSeed.Value}";
                var roll = Main.Multiplayer.ValueGenerator.Range(SeedLifetime.Area, identifier, minInclusive, maxExclusive);
